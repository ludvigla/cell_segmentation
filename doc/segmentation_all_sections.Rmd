...
PlotFeatures <- function (
  ...
  if (!is.null(group.by) & !is.null(color.by)) {
    ...
    region.colors <- setNames(region.colors, gg_color_hue(length(region.colors)))
  } else {
    region.colors <- NULL
  }
  ...
  if (!is.null(group.by)) {
    p <- p +
      geom_point(data = gg, aes_string("warped_x", "imdims[2] - warped_y", fill = group.by), shape = 21, size = pt.size, alpha = pt.alpha) +
