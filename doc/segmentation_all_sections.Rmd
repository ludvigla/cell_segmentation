...
# Load Seurat object
brain.merge <- readRDS("../data/merged.Rds")
brain.merge
...
names(convert.colors) <- unique(as.character(brain.merge$seurat_clusters))
brain.merge$colors <- convert.colors[as.character(brain.merge$seurat_clusters)]

regi <- readRDS(file = "../R_objects/registration_HE_V9")
...
PlotFeatures <- function (
  ...
  imdims.png <- (image_read(PNG.img) %>% image_info())[2:3] %>% as.numeric()
  imdims.tif <- ((image_read(TIF.img) %>% image_info())[2:3] %>% as.numeric())/resize
  ...
  if (!is.null(group.by) & !is.null(color.by)) {
    ...
    region.colors <- sapply(1:nrow(object), function(i) {
      setNames(object@meta.data[i, color.by], object@meta.data[i, group.by])
    })
    region.colors <- region.colors[!duplicated(names(region.colors))]
  ...
  # Load reference image and scale to fit dimesnions of reference HE image
  ...
  if (fill.atlas) {
    p <- p + geom_polygon(data = d, aes(x, imdims.tif[2] - y, group = region_combined), fill = d$color, alpha = 1, color = NA)
  }
  if (color.atlas) {
    p <- p + geom_polygon(data = d, aes(x, imdims.tif[2] - y, group = region_combined), alpha = 0, color = "black")
  }
...
# Plot all clones
PlotFeatures(object = brain.merge, 
             st.object = st.object.v9tov12, 
             selected.clones = "0", 
             polygon.data.frame = d, # Polygons extracted from registration object
             PNG.img = "~/Michael_Ratz/cell_segmentation/data/spaceranger/v9/spatial/tissue_hires_image.png", # Image used for alignment with STUtility (check Staffli object : st.object)
             TIF.img = "~/Michael_Ratz/cell_segmentation/hematoxylin_and_eosin_test/images/he_v9_extended.tif", # Image used for registration (modified version of original HE image, before running spaceranger)
             pt.alpha = 0.3)
...
PlotFeatures(object = brain.merge, 
             st.object = st.object.v9tov12, 
             selected.clones = "0", 
             polygon.data.frame = d,
             group.by = "seurat_clusters",
             PNG.img = "~/Michael_Ratz/cell_segmentation/data/spaceranger/v9/spatial/tissue_hires_image.png", 
             TIF.img = "~/Michael_Ratz/cell_segmentation/hematoxylin_and_eosin_test/images/he_v9_extended.tif",
             pt.alpha = 0.4, 
             fill.atlas = T)
...
PlotFeatures(object = brain.merge, 
             ...
             selected.clones = "0", 
             polygon.data.frame = d, # Polygons extracted from registration object
             PNG.img = "~/Michael_Ratz/cell_segmentation/results/reference_images/Dapi_aligned.jpeg", # Image used for alignment with STUtility (check Staffli object : st.object)
             TIF.img = "~/Michael_Ratz/cell_segmentation/hematoxylin_and_eosin_test/images/Neun_new_small_canvassize.tif", # Image used for registration (modified version of original HE image, before running spaceranger)
             pt.alpha = 0.3)
