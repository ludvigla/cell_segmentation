...
setwd("~/Michael_Ratz/hematoxylin_and_eosin_test/")
...
#   for the spots later.
imdapi <- image_read("images/dapi_edited.tif")
...
image_write(imdapi, path = "images/dapi_edited_small_canvassize.tif")
...
imdapi_mod <- "images/dapi_edited_small_canvassize.tif"
quartz()
seg <- segment("images/dapi_edited_small_canvassize.tif", filter = myfilter, display = TRUE)
...
quartz()
regi <- registration(imdapi_mod, filter = myfilter, coordinate = coord, right.hemisphere =  TRUE, resize = 0.4)
...
regi$correspondance <- readRDS("correspondance.points")
...
regi <- registration(imdapi_mod, filter = myfilter, coordinate = coord, right.hemisphere =  TRUE, correspondance = regi)

# Ludvig: 
#   You can export the correspondance points as a data.frame. It's probably good to keep!
# Save correspondance points 
# saveRDS(regi$correspondance, file = "correspondance.points")
...
quartz.save('registration_results.pdf', type='pdf', width = 12, height = 4)
...
tr <- rotate(-90, center.cur = c(4464/2, 4464/2)) # The images were downscaled by a factor 0f 0.5 so the new image center is 4464/2, 4464/2
